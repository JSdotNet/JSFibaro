...
then
    ...
    function getMaxLevel() -- TODO (base on lux level ???)
        ...
        if (activity == "Going to Sleep" or activity == "Sleeping" or activity == "Waking")
    ...
    function setLevel(deviceIds, dimlevel)
        for key, deviceID in pairs(deviceIds) do
            local currentLevel = fibaro:call(deviceID, "getValue"); 
            
            if (currentLevel < dimlevel) then
            ...
            else if (debug) then fibaro:debug(scene .. " - Device (" .. fibaro:getName(deviceID) .. ") already at required dim level"); end
        end
    end
